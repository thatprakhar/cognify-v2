...
export const Accordion: React.FC<AccordionProps> = ({ allowMultiple = false, children }) => {
    ...
    return (
        <div className="w-full border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden divide-y divide-zinc-200 dark:divide-zinc-800">
            {items.map((item, idx) => {
                ...
                return (
                    <div key={idx} className="bg-white dark:bg-black">
                        <button
                            onClick={() => toggle(idx)}
                            className="w-full flex items-center justify-between p-4 text-left font-medium hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors"
                        >
                            <span>{title}</span>
                            <motion.div
                        ...
                        <AnimatePresence initial={false}>
                            {isOpen && (
                                ...
                                >
                                    <div className="p-4 pt-0 border-t border-zinc-100 dark:border-zinc-900">
                                        {item}
                                    </div>
                                </motion.section>
                            )}
