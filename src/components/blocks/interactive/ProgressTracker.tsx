...
export const ProgressTracker: React.FC<ProgressTrackerProps> = ({ steps = [], currentStep = 0 }) => {
 ...
 <div className="absolute left-0 top-1/2 transform -translate-y-1/2 w-full h-1 bg-zinc-200 z-0"></div>
 <div
 ...
 let circleClass = "bg-white border-zinc-300 text-zinc-400";
 if (isCompleted) {
 circleClass = "bg-blue-600 border-blue-600 text-white";
 } else if (isCurrent) {
 circleClass = "bg-white border-blue-600 text-blue-600 ring-4 ring-blue-100 ";
 }
 ...
 <span className={`text-xs font-medium absolute top-10 whitespace-nowrap ${isCurrent ? 'text-blue-600 ' : 'text-zinc-500'}`}>
 {step}
 </span>
 </div>
 );
